...
`scripts/spec-check.ts` の仕様。Spec とソースの乖離を機械的に検出し、動作確認完了時のゲートとして使う。
...
| 用語 | 意味 |
|---|---|
| 定義 | `docs/spec/*.md` のチェックボックス行に現れる SPEC-ID |
...
| apiDrift | `SPEC-API.md` と `server/routes/` の間で片側にしか存在しないエンドポイント |

## 受け入れ基準

- [x] `SPEC-FLOW-001` docs/spec/*.md のチェックボックス行から `SPEC-<領域>-<3桁>` 形式の ID を定義として抽出する
...
- [x] `SPEC-FLOW-010` SPEC-API.md のエンドポイント記述と server/routes の登録ルートを突き合わせ、片側のみのものを apiDrift として報告する
