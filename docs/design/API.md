...
| エンドポイント | 説明 | 主なクエリ |
|---|---|---|
| `GET /api/overview` | 全体集計（総トークン・推定コスト・モデル別・日次系列・プロジェクト一覧） | `from` `to`（YYYY-MM-DD） |
| `GET /api/projects` | プロジェクト一覧（セッション数・トークン・推定コスト・最終更新） | — |
| `GET /api/projects/:id` | プロジェクト詳細（日次モデル別系列・セッション一覧） | `from` `to` |
...
| `GET /api/stats/tokens` | モデル別 × 日別トークン | `from` `to` |
| `GET /api/stats/tools` | ツール別呼び出しランキング | — |
| `GET /api/stats/agents` | subagent / skill 別の起動実績 | — |
